<main class="inner-container grid grid-cols-1 gap-8 py-28">
	<!-- Blog List -->
	<div class="space-y-12 [&>*]:border-b [&>*]:border-b-gray-border [&>*]:pb-12">
		<% loop $BlogPosts %>
			<div>
				...
				<a class="animated-underline after:h-[1px]" href="$Link" title="By $Author.Name">By $Author.Name</a>
				<span>/</span>
                    <% loop $Categories %>
					<a class="animated-underline after:h-[1px]" href="$Up.Link" title="$Name">
						$Name<% if not $IsLast %>, <% end_if %>
					</a>
				<% end_loop %>
			</div>
				<h2 class="mb-4 font-playfair text-4xl text-heading">
					<a class="transition-colors hover:text-gold" href="$Link" title="$Title">
						$Title
					</a>
				</h2>
		...
		<% if $BlogPosts.MoreThanOnePage %>
			<div class="my-8 flex justify-center space-x-4">
				<% if $BlogPosts.NotFirstPage %>
					<a class="px-4 py-2 text-heading border border-heading transition-colors hover:bg-navy hover:text-white" title="Previous" href="$BlogPosts.PrevLink">Previous</a>
				<% end_if %>
				...
				<% if $BlogPosts.NotLastPage %>
					<a class="px-4 py-2 text-heading border border-heading transition-colors hover:bg-navy hover:text-white" title="Next" href="$BlogPosts.NextLink">Next</a>
				<% end_if %>
			</div>
		<% end_if %>
	</div>
